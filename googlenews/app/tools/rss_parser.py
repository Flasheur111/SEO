...
class RssParser:
    ...
    def get_news_previews(self, count=10, category='A la une', lang='fr'):
        ...
        article_previews = []
        for post in news_feed.entries:
            article_preview = {'title': BeautifulSoup(post.title).get_text(),
                               'text': BeautifulSoup(post.summary).get_text()}
            article_previews.append(article_preview)
